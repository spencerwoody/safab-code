...
source("R/sims_create_intervals.R")

Date <- lubridate::today()

filename <- sprintf("output/bh-joint/bh-joint-%s.Rdata", Date)

###############################################################################
                                        #            Set parameters           #
###############################################################################

source("setpars-bh-joint.R")
...
while (k <= nMC) {
  ...
  ## BH decision rule
  yMeanK <- rowMeans(yMatK)

  zVec <- yMeanK / sigma_mean

  ## p-values
  pvec <- 2 * pnorm(-abs(zVec))

  ## Order (indices) of p-values, and sorted p-values
  pvecOrder <- order(pvec)
  pvecSort <- sort(pvec)
  
  ## Which values to keep
  selectIdx <- pvecOrder[(pvecSort / seq_along(pvecSort)) <= (qstar / Nj)]

  if (length(selectIdx) == 0) {
    next
  }

  t <- max(abs(yMeanK[-selectIdx]))
  tVec[k] <- t
...
simsDfCat <- plyr::rbind.fill(simsDfList) 

save(list=ls(), file = filename)

load("output/bh-joint/bh-joint-2019-10-14.Rdata")
...
summaryDfXtable <- xtable(summaryDf, floating = FALSE,
                          booktabs = TRUE,
                          caption = "Data-dependent thresholding via Benjamini-Hochberg procedure, joint selection")
