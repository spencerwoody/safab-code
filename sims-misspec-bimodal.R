...
filename <- sprintf("output/misspec-bimodal/misspec-bimodal-output-%s.Rdata", Date)

###############################################################################
                                        #            Set parameters           #
###############################################################################

source("setpars-misspec-bimodal.R")
...
wsaFABo <- make_w_theta(marginal_fun = marginal_nl_joint, 
                        sigma = sigma_mean, tau = tau,  t = t,
                        p = p, prop = prop, mu = mu,
...
while (k <= nMC) {
  ...
  thetaVecK <- c(rnorm(Nj *      prop, -mu, tau),
                 rnorm(Nj * (1 - prop), mu, tau))
...
load("output/misspec-bimodal/misspec-bimodal-output-2019-10-14.Rdata")
...
summaryDfXtable <- xtable(summaryDf, floating = FALSE,
                          booktabs = TRUE,
                          caption = "Misspecified prior, bimodal alternative density case, joint selection")
