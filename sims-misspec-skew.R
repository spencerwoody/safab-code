...
filename <- sprintf("output/misspec-skew/%s-job-%s.Rdata", Date, jobJpad)

###############################################################################
                                        #            Set parameters           #
###############################################################################

source("setpars-misspec-skew.R")
...
wsaFABo <- make_w_theta(marginal_fun = marginal_exp_joint, 
                        sigma = sigma_mean, lambda = lambda,  t = t,
                        p = p, mu = mu,
...
wsaFABo$w[wsaFABo$theta < 0.5 & wsaFABo$w > 0.8] <- min(wsaFABo$w)
...
while (k <= nMC) {
  ...
  thetaVecK <- mu + rexp(Nj, rate = lambda)
  thetaVecK <- thetaVecK * rbinom(Nj, 1, p)
  ...
  while(foldCount <= numFolds) {
    ...
    wsaFABe <- make_w_theta(marginal_fun = marginal_ss_joint,
    ...
    wsaFABn <- make_w_theta(marginal_fun = mypr$my_fit_jnt,
...
load("output/misspec-skew/2019-10-14-job-NA.Rdata")
...
summaryDfXtable <- xtable(summaryDf, floating = FALSE,
                          booktabs = TRUE,
                          caption = "Misspecified prior, skew alternative density case, joint selection")
...
lenDf <- rbind(
  data.frame(meanlen = meanlengthVecO,
             mode = "Oracle"),
  data.frame(meanlen = meanlengthVecE,
             mode = "Parametric empirical Bayes"),
  data.frame(meanlen = meanlengthVecN,
             mode = "Nonparametric empirical Bayes"),
  data.frame(meanlen = meanlengthVecU,
             mode = "UMAU")
) %>%
  mutate(mode = factor(mode, levels = c("Oracle", "UMAU", "Parametric empirical Bayes",
                                        "Nonparametric empirical Bayes")))

## lenPlot <- ggplot(lenDf) +
##   geom_density(aes(meanlen, lty = mode), size = 1) +
...
lenPlot <- ggplot(lenDf) +
  geom_density(aes(meanlen, fill = mode), alpha = 0.5) +
  theme(legend.position = "bottom") +
  scale_fill_brewer("Method", palette = "Greys") +
  labs(## title = "Comparison of confidence set efficiency",
       ## subtitle = "Case of misspecified prior with skewed alternative density",
       x = "Average size of confidence set") +
  guides(fill = guide_legend(nrow = 2))

lenPlot

ggsave("lenplot.pdf", lenPlot, width = 6.6, height = 4.2, units = "in")

ggsave("lenplot.eps", lenPlot, width = 6.6, height = 4.2, units = "in", device=cairo_ps)
