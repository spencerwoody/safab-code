...
filename <- sprintf("output/nonpar-joint/nonpar-joint--%s.Rdata", Date)

###############################################################################
                                        #            Set parameters           #
###############################################################################

source("setpars-nonpar-joint.R")
...
wsaFABo <- make_w_theta(marginal_fun = marginal_ss_joint, 
...
while (k <= nMC) {
  ...
  while(foldCount <= numFolds) {
    ...
    eList <- try(
      saFAB(wsaFABe$theta, wsaFABe$w, t, sigma_mean, verbose = FALSE,
            yMin = -5, yMax = 5, yNum = 1e3)
    )
    ...
    if (inherits(eList, "try-error") | inherits(nList, "try-error")) {
      break
    }
...
load("output/nonpar-joint/nonpar-joint--2019-10-15.Rdata")
...
sqrt(mean(meancoverVecO) * (1 - mean(meancoverVecO)) / nMC)
sqrt(mean(meancoverVecC) * (1 - mean(meancoverVecC)) / nMC)
sqrt(mean(meancoverVecE) * (1 - mean(meancoverVecE)) / nMC)
sqrt(mean(meancoverVecN) * (1 - mean(meancoverVecN)) / nMC)
sqrt(mean(meancoverVecU) * (1 - mean(meancoverVecU)) / nMC)
...
summaryDfXtable <- xtable(summaryDf, floating = FALSE,
                          booktabs = TRUE,
                          caption = "Well specified prior case, joint selection")

print(summaryDfXtable, include.rownames=FALSE)


###############################################################################
                                        #        Plot density of sizes        #
###############################################################################
...
lenPlot <- ggplot(lenDf) +
  geom_density(aes(meanlen, lty = mode))
